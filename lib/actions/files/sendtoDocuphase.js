...
async function processTrigger(msg, cfg, next, snapshot) {
    var that = this;
    const { PodioItemId, UniqueId,FirstName,LastName,LocationRef,Agency,FileId,DocuPhaseURL } =  msg.body;;
    ...
    if (!FileId) {
        throw new Error('Field Id field is required');
    }
    if (!DocuPhaseURL) {
        throw new Error('Docuphase Url field is required');
    }

    axios.post('https://sync.thatapp.io/api/v2/docuphase', {
