...
reformData = async function (item, podio) {
    for (const key of Object.keys(item)) {
        ...
        if (typeof value.embed === "string") {
            const url = value.embed.trim();
            // "[]" (clear sentinel) or empty → clear the field instead of POSTing
            // a non-URL to /embed/, which Podio rejects (the source of link errors).
            if (url === "" || url === "[]") {
                item[key] = [];
                continue;
            }
            const resolvedUrl = await podio.post('/embed/', { url });
