...
exports.process = async function createItem(msg, cfg) {
    ...
    var resolvedApp =  function resolveAppFields(app) {
    ...
    console.log(resolvedApp);
    var transformedApp = transformFields(resolvedApp);

    var item_ = insertItem(transformedApp);
    ...
    async function insertItem(data, files) {
        console.log(data);
        if (!_.isEmpty(files)) {
        ...
        function returnItem(newItemData) {
         ...
         var items = await podio.post('/item/app/' + cfg.appId, data);
